...
import { IProfile, IPromptCardProps } from "@/types/types";
...
const PromptCard = ({
  ...
  return (
    <div className="prompt_card">
        ...
        >
          ...
          <div className="flex flex-col">
            <h3 className="font-satoshi font-semibold text-gray-900">
              {post?.creator?.username as string}
            </h3>
            <p className="font-inter text-sm text-gray-500">
              {post?.creator?.email as string}
            </p>
          </div>
        </div>
      ...
      {(session?.user as IProfile)?.id === post?.creator?._id &&
        pathName === "/profile" && (
